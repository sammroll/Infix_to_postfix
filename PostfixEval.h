...
int PostfixEval::compute(int left, int right, char op) const {
	...
	case '^':
		if (left == 0 && right == 0) {
			throw ExpressionError("postfixEval: 0^0 indeterminate form error");
		}
		ans = pow(left, right);
		break;
	}
...
int PostfixEval::evaluate() {
	...
	for (i = 0; i < postfixExpression.length(); i++) {
		...
		if (isdigit(ch) && ch != ' ') {
			...
			//make the computation after getting left and right
			getOperands(left, right);
			operandStack.push(compute(left, right, ch));
		}
		else if (!isspace) {
			throw ExpressionError("PostfixEval: Invalid input");
		}
	}
	eval = operandStack.topAndPop();
